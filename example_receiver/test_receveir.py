...
# This holds the absolute latest data from the ESP32
state = {
    "roll": 0.0, "pitch": 0.0, "yaw": 0.0,
    "qw": 1.0, "qx": 0.0, "qy": 0.0, "qz": 0.0,
    "joy_x": 0.0, "joy_y": 0.0, "sw": 0
}
...
def joy_notification_handler(sender, data):
    state["joy_x"], state["joy_y"], state["sw"] = struct.unpack('<2fB', data)
    ...
    async with BleakClient(device) as client:
        ...
        try:
            while True:
                ...
                # This prevents flickering and overwrites the text cleanly
                ...
                sys.stdout.write(f"[JOY]   X: {state['joy_x']:7.2f} | Y: {state['joy_y']:7.2f}     | SW:  {state['sw']}\n")
                sys.stdout.write("==================================================\n")
                sys.stdout.flush()
                
                # Update the UI at 20Hz (every 50ms)
                # The BLE handlers will update the dictionary much faster in the background
                await asyncio.sleep(0.05)
                
        except KeyboardInterrupt:
            # Move cursor past the table so we don't overwrite it on exit
            print("\n\nDisconnecting...")
