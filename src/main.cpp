...
// New Single-Axis + Switch
const int PIN_Z = 4;
const int PIN_SW2 = 10;
...
int centerX = 2048;
int centerY = 2048;
int centerZ = 2048; // Center for the new axis
...
// Expanded struct for Joystick + Gripper Trigger
struct JoystickData {
  float x;
  float y;
  float z;       // New analog axis
  uint8_t sw;    // Original switch
  uint8_t sw2;   // New switch
} __attribute__((packed));
...
void calibrateJoystick() {
  ...
  Serial.println("Calibrating joystick axes... Do not touch.");
  
  for (int i = 0; i < CALIBRATION_SAMPLES; i++) {
    sumX += analogRead(PIN_X);
    sumY += analogRead(PIN_Y);
    sumZ += analogRead(PIN_Z);
    delay(10);
  }

  centerX = sumX / CALIBRATION_SAMPLES;
  centerY = sumY / CALIBRATION_SAMPLES;
  centerZ = sumZ / CALIBRATION_SAMPLES;

  Serial.printf("Calibration complete. Centers -> X: %d | Y: %d | Z: %d\n", centerX, centerY, centerZ);
}
...
void setup() 
{
  ...
  pinMode(PIN_SW2, INPUT_PULLUP); // Init the new switch
...
void loop() 
{
  ...
  int rawZ = analogRead(PIN_Z); // Read new axis
  
  int swState = !digitalRead(PIN_SW);
  int sw2State = !digitalRead(PIN_SW2); // Read new switch
